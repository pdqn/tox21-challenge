...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 17 20:09:56 2017
...
#target="SR.ARE"
    ...
    x_train=x_tr_dense[rows_tr]
    y_train=y_tr[target][rows_tr]
    y_train=pd.get_dummies(y_train)
    x_test=x_te_dense[rows_te]
...
x_train, y_train, x_test, y_test = _dataset(targets[3])
n = x_train.shape[0]
batches_train_x = [x_train[k:k+batch_size] for k in xrange(0, n,batch_size)]
batches_train_y = [y_train[k:k+batch_size] for k in xrange(0, n,batch_size)]

n = x_test.shape[0]
batches_test_x = [x_test[k:k+batch_size] for k in xrange(0, n,batch_size)]
batches_test_y = [y_test[k:k+batch_size] for k in xrange(0, n,batch_size)]
...
b=y_test #tf.reduce_max(y_te, reduction_indices=[1])
...
tf.summary.histogram("weights3", weights['h3'])
tf.summary.histogram("weights4", weights['h4'])
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            _, c = sess.run([optimizer, cost], feed_dict={x: batch_x,
                                                          y: batch_y, dropoutRate: 0.01, is_training:True})
        ...
        if epoch % display_step == 0:
            ...
            batch_x_test, batch_y_test = x_test,y_test #batches_test_x[i],batches_test_y[i]
            ...
            valAcc = sess.run(accuracy, feed_dict = {x:batch_x_test, y : batch_y_test,dropoutRate : 0.0, is_training:False})
